...
#define PACKET_IDENTITY_MAX					REG_MASK_6_BIT
#define PACKET_IDENTITY_MASK				REG_MASK_6_BIT
...
)
{
	...
	tp_Control = &m_t_Control[t_Device];

	//Check if control has already been initialized
	if (REG_GET_BIT(tp_Control->t_Flag, DEVCOMM_CONTROL_FLAG_VALID) != 0)
	{
		return FUNCTION_FAIL;
	}

	//Check if maximum packet length is valid or not
	if (tp_Profile->t_PacketLengthMax < sizeof(devcomm_packet_head) + 
		sizeof(devcomm_segment_head))
	{
		return FUNCTION_FAIL;
	}
	...
	tp_Callback->fp_Memcpy((uint8 *)&tp_Control->t_Profile, 
		(const uint8 *)tp_Profile, sizeof(devcomm_profile));
	tp_Callback->fp_Memcpy((uint8 *)&tp_Control->t_Callback, 
		(const uint8 *)tp_Callback, sizeof(devcomm_callback));

	REG_SET_BIT(tp_Control->t_Flag, DEVCOMM_CONTROL_FLAG_VALID);
...
)
{
	...
	if ((t_PacketLength > m_t_Control[t_Device].t_Profile.t_PacketLengthMax) ||
		(t_PacketLength < sizeof(devcomm_packet_head) + 
		sizeof(devcomm_segment_head)))
	{
		return FUNCTION_FAIL;
	}
	...
	for (i = 0; i < DEVCOMM_LINK_COUNT_MAX; i++)
	{
		if (((REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_VALID) != 0) &&
			(tp_Link->t_Address == t_Address)) ||
			(REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_VALID) == 0))
		{
			break;
		}
	...
	if (REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_VALID) == 0)
	{
		REG_SET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_VALID);
		tp_Link->t_Address = t_Address;
...
)
{
	...
	//Return if no valid link is found
	if (i == DEVCOMM_LINK_COUNT_MAX)
	{
		return FUNCTION_FAIL;
	}
	...
	if (REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT) != 0)
	{
		return FUNCTION_FAIL;
	}
	...
	tp_Profile = &tp_Control->t_Profile;

	if (t_Mode == DEVCOMM_MODE_ACKNOWLEDGEMENT)
	{
		REG_SET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT);
	}
	else
	{
		REG_CLEAR_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT);
	}

	tp_Link->u16_TimeoutTimer = 0;
	tp_Link->u16_RetryCounter = 0;
	tp_Link->tp_SendPacketHead = (devcomm_packet_head *)tp_Link->u8_SendBuffer;
	tp_Link->tp_SendSegmentHead = 
		(devcomm_segment_head *)(tp_Link->u8_SendBuffer + 
		sizeof(devcomm_packet_head));
	tp_Link->tp_SendPacketHead->t_SourceAddress = tp_Profile->t_Address;
	...
	DevComm_ReloadSendBuffer(tp_Link, &tp_Control->t_Callback, u8p_Data, 
		t_Length);
...
)
{
	...
	t_DataLength = tp_PacketHead->t_Length - 
		(sizeof(devcomm_packet_head) + sizeof(devcomm_segment_head));
	...
	u8p_Buffer = tp_Link->u8_ReceiveBuffer + 
		(sizeof(devcomm_packet_head) + sizeof(devcomm_segment_head));
	tp_Callback->fp_Memcpy(u8p_Data, u8p_Buffer, t_DataLength);
	u8p_Buffer += tp_PacketHead->t_Length - sizeof(devcomm_segment_head);
	u8p_Data += t_DataLength;
	t_PayloadLength = t_DataLength;

	if (REG_GET_BIT(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_MODE) == 0)
	{
	...
	*tp_Length -= t_DataLength;
	...
	while ((REG_GET_BIT(tp_PacketHead->u8_Control, 
		PACKET_CONTROL_OFFSET_ENDING) == 0) && (*tp_Length > 0))
	{
		tp_PacketHead = (devcomm_packet_head *)((uint8 *)tp_PacketHead +
			...
			*tp_Value = DEVCOMM_STATE_IDLE; 
			...
			for (i = 0; i < DEVCOMM_LINK_COUNT_MAX; i++)
			{
				if ((REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_VALID) != 0) &&
					(tp_Link->t_Address == t_Address))
				{
					if ((REG_GET_BIT(tp_Link->t_Flag, 
						DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT) != 0) ||
						(tp_Link->t_SendPacketCount > 0))
					{
...
)
{
	...
	if (tp_Link->t_SendPacketCount > 0)
	{
		tp_Link->tp_SendPacketHead = 
			(devcomm_packet_head *)((uint8 *)tp_PacketHead + 
			(tp_PacketHead->t_Length));
		DevComm_SendPacket(t_Device, tp_Link);
	}
	else
...
)
{
	...
	if (REG_GET_BIT(m_t_Control[t_Device].t_Flag, 
		DEVCOMM_CONTROL_FLAG_VALID) == 0)
	{
		return FUNCTION_FAIL;
	}
...
)
{
	...
	devcomm_int t_PacketCount;


	t_PacketPayload = tp_Link->t_PacketLength - sizeof(devcomm_packet_head);

	if ((t_Length > (devcomm_int)(~(0)) - 
		(t_PacketPayload + sizeof(devcomm_segment_head) - 1)))
	{
		return FUNCTION_FAIL;
	}	

	t_PacketCount = (sizeof(devcomm_segment_head) + t_Length + 
		t_PacketPayload - 1) / t_PacketPayload;

	//Check if length of payload data is out of range
	if ((t_PacketPayload + sizeof(devcomm_packet_head)) * t_PacketCount >
		DEVCOMM_LINK_BUFFER_SIZE)
	{
		return FUNCTION_FAIL;
	}
...
)
{
	...
	m_t_Control[t_Device].t_Callback.fp_WriteDevice(t_Device, 
		(const uint8 *)tp_PacketHead, tp_PacketHead->t_Length);
...
)
{
	...
	if (tp_PacketHead->u8_Checksum != 
		tp_Callback->fp_GetCRC8((const uint8 *)tp_PacketHead,
	...
	if (tp_PacketHead->t_Offset == 0)
	{
		...
		u8p_Buffer = tp_Link->u8_ReceiveBuffer;
		tp_Link->tp_ReceiveSegmentHead = (devcomm_segment_head *)(u8p_Buffer + 
			sizeof(devcomm_packet_head));
	}
	else
	{
		...
		if (REG_READ_FIELD(tp_ReceivePacketHead->u8_Control, 
			PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK) !=
			REG_READ_FIELD(tp_PacketHead->u8_Control,
			PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK))
		{
			return DevComm_DiscardPacket(t_Device, tp_Control, t_Length);
		}
		...
		//Check if length of packet is out of range
		if (tp_PacketHead->t_Length > (tp_Link->u8_ReceiveBuffer + 
			DEVCOMM_LINK_BUFFER_SIZE) - ((uint8 *)tp_ReceivePacketHead +
			tp_ReceivePacketHead->t_Length))
		{
			return DevComm_DiscardPacket(t_Device, tp_Control, t_Length);
		}

		//Check if offset is match or not
		if (tp_ReceivePacketHead->t_Offset + tp_ReceivePacketHead->t_Length -
			sizeof(devcomm_packet_head) != tp_PacketHead->t_Offset)
		{
			return DevComm_DiscardPacket(t_Device, tp_Control, t_Length);
		}

		u8p_Buffer = (uint8 *)tp_ReceivePacketHead +
			tp_ReceivePacketHead->t_Length;
	}

	if (tp_Callback->fp_ReadDevice(t_Device, u8p_Buffer, &t_Length) != 
		FUNCTION_OK)
	{
		return FUNCTION_FAIL;
	}
	...
	if (REG_GET_BIT(tp_Link->tp_ReceivePacketHead->u8_Control, 
...
)
{
	...
	u16_Checksum = tp_Callback->fp_GetCRC16(u8p_Buffer,
		tp_PacketHead->t_Length - (sizeof(devcomm_packet_head) + 
		sizeof(devcomm_segment_head)), u16_Checksum);
	u8p_Buffer += tp_PacketHead->t_Length - sizeof(devcomm_segment_head);
	...
	if (REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT) != 0)
	{
		...
		while (u16_RetryCounter <= tp_Link->u16_RetryCounter)
		{
			//Check if the packet being sent is acknowledged or not
			if (tp_SegmentHead->u16_Acknowledgement ==
				tp_Link->u16_Acknowledgement[u16_RetryCounter])
			{
				REG_CLEAR_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT);
	...
	if (t_PayloadLength > 0)
	{
		...
		if ((REG_READ_FIELD(tp_PacketHead->u8_Control, 
			PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK) != 
			tp_Link->u8_ReceiveIdentity) ||
			(REG_READ_FIELD(tp_PacketHead->u8_Control,
			PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK) == 0))
		{
			tp_Link->u8_ReceiveIdentity = REG_READ_FIELD(tp_PacketHead->u8_Control,
				PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK);
			...
			tp_Link->t_ReceivePacketCount++;
			tp_Callback->fp_HandleEvent(t_Device, tp_PacketHead->t_SourceAddress,
				tp_SegmentHead->t_SourcePort, tp_SegmentHead->t_TargetPort, 
...
)
{
	...
	return tp_Callback->fp_ReadDevice(t_Device, tp_Control->u8_PacketBuffer, 
		&t_PacketLength);
}
...
)
{
	...
	devcomm_int t_PacketLength;
	devcomm_callback *tp_Callback;
	devcomm_control *tp_ForwardControl;
	...
	if (tp_Callback->fp_ReadDevice(t_Device, tp_Control->u8_PacketBuffer, 
		&t_PacketLength) != FUNCTION_OK)
	{
		return FUNCTION_FAIL;
	}

	tp_ForwardControl = m_t_Control;

	//Forward packet to all other devices except for the current device
	for (i = 0; i < DEVCOMM_DEVICE_COUNT_MAX; i++)
	{
		if ((i != t_Device) && (t_PacketLength <= 
			tp_ForwardControl->t_Profile.t_PacketLengthMax))
		{
			tp_Callback = &tp_ForwardControl->t_Callback;	
			tp_Callback->fp_WriteDevice(i, tp_Control->u8_PacketBuffer, 
				t_PacketLength);
		}
...
)
{
	...
	{
		tp_SegmentHead->u16_Acknowledgement = 
			tp_Link->tp_ReceiveSegmentHead->u16_Checksum;
	}
	else
	{
	...
	tp_SegmentHead->u16_Checksum = 
		tp_Callback->fp_GetCRC16((const uint8 *)tp_SegmentHead, 
		sizeof(devcomm_segment_head) - sizeof(tp_SegmentHead->u16_Checksum),
		CHECKSUM_BASE_CRC16);
	tp_SegmentHead->u16_Checksum = tp_Callback->fp_GetCRC16(u8p_Data, t_Length,
		tp_SegmentHead->u16_Checksum);
	tp_PacketHead->u8_Control = REG_READ_FIELD(tp_PacketHead->u8_Control,
		PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK);

	if (tp_PacketHead->u8_Control < (uint8)PACKET_IDENTITY_MAX)
	{
		if (REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_IDENTITY) != 0)
		{
		...
		else
		{
			...
			REG_SET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_IDENTITY);
		}
	}
	else
	{
	...
	if (REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT) != 0)
	{
		tp_Link->u16_Acknowledgement[tp_Link->u16_RetryCounter] = 
			tp_SegmentHead->u16_Checksum;
	}
	else
	{
		REG_SET_BIT(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_MODE);
	}
	...
	tp_PacketHead->t_Length = sizeof(devcomm_packet_head) + 
		sizeof(devcomm_segment_head) + t_DataLength;
	...
	while (t_Length > 0)
	{
		tp_PacketHead->u8_Checksum = 
			tp_Callback->fp_GetCRC8((const uint8 *)tp_PacketHead,
			sizeof(devcomm_packet_head) - sizeof(tp_PacketHead->u8_Checksum), 
			CHECKSUM_BASE_CRC8);
		tp_Callback->fp_Memcpy(u8p_Buffer, (const uint8 *)tp_PacketHead,
			sizeof(devcomm_packet_head));
	...
	REG_SET_BIT(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_ENDING);
	tp_PacketHead->u8_Checksum = 
		tp_Callback->fp_GetCRC8((const uint8 *)tp_PacketHead,
		sizeof(devcomm_packet_head) - sizeof(tp_PacketHead->u8_Checksum), 
		CHECKSUM_BASE_CRC8);
...
)
{
	...
	{
		t_SendPacketCount++;
		u16_Acknowledgement = tp_SegmentHead->u16_Acknowledgement;
		...
		if (t_SendPacketCount == tp_Link->t_SendPacketCount)
		{
			tp_SegmentHead->u16_Checksum = u16_Checksum;
		}
		else
		{
			tp_SegmentHead->u16_Acknowledgement = u16_Acknowledgement;
		}
	}
...
)
{
	...
	if ((REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_VALID) == 0) ||
		(REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT) == 0))
	{
		return;
	}
